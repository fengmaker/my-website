...
class Solution:
    def sumEvenGrandparent(self, root: Optional[TreeNode]) -> int:
        ...
        def dfs(node, parent_even: bool, grandparent_even: bool) -> None:
            nonlocal answer
            if node is None:
                return
            if grandparent_even:
            ...
            current_even = node.val % 2 == 0
            dfs(node.left, current_even, parent_even)
            dfs(node.right, current_even, parent_even)
...
```text
def dfs(node):
    if not node: return 0  # 按具体题目的 base case 调整
